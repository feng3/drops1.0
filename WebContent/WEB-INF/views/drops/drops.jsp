			...
			<div class="leftpanelinner">
				...
				<ul class="nav nav-pills nav-stacked nav-bracket">
					<li class="active"><a href="./drops.do?method=getDrops"> <i
							class="fa fa-home"></i> <span>文章分类</span>
					</a></li>
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				...
				<% 
					...
					if("dev".equals(category)){
						...
						subTitle = "工具";
					}else if("tips".equals(category)){
						className = "fa fa-leaf";
						subTitle = "";
					}else if("papers".equals(category)){
				...
				%>
					<i class="<%=className %>"></i>  <%=category %><span><%=subTitle %></span>
				</h2>
			</div>
			...
			<div class="contentpanel">
				...
				<div class="row">
					...
					<div class="col-sm-12">
						...
						<div class="table-responsive">
							<table class="table table-success table-striped mb30">
								<thead>
									<tr>
										<th width="15%">发布时间</th>
										<th width="50%">标题</th>
								...
								<tbody>
									<c:forEach items="${page.list }" var="drop">
										<tr>
											<td>${fn:substring(drop.createTime,0,10) }</td>
											<td><a href="./drops.do?method=getDrop&id=${drop.id}"
												target="_blank">${drop.title }</a></td>
											<td><a
												href="./drops.do?method=getDrops&author=${drop.author }">${drop.author }</a></td>
											<td><a
												href="./drops.do?method=getDrops&category=${drop.category }">${drop.category }</a></td>
											<td><a
												href="./drops.do?method=getDrops&sourceSite=${drop.sourceSite }">${drop.sourceSite }</a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
				...
				<div class="row">
					...
					<div class="col-md-12">
					  <h3>
						第<font color="#004B97">${page.pageNo }</font>页/共<font color="#004B97">${page.totalPageNumber}</font>页
					  
						<c:if test="${page.hasPrev }">
							<a class="btn btn-darkblue" href="drops.do?method=getDrops&category=${param.category}&author=${param.author }&sourceSite=${param.sourceSite }">首页</a>
							<a class="btn btn-primary" href="drops.do?method=getDrops&category=${param.category}&author=${param.author }&sourceSite=${param.sourceSite }&pageNo=${page.prevPage }">上一页</a>
						</c:if>
						<c:if test="${page.hasNext }">
							<a class="btn btn-darkblue" href="drops.do?method=getDrops&category=${param.category}&author=${param.author }&sourceSite=${param.sourceSite }&pageNo=${page.nextPage }">下一页</a>
							<a class="btn btn-primary" href="drops.do?method=getDrops&category=${param.category}&author=${param.author }&sourceSite=${param.sourceSite }&pageNo=${page.totalPageNumber }">末页</a>
						</c:if>
					  </h3>
 						<h3>
							共<font color="#930000">${page.totalItemNumber }</font>条记录&nbsp;&nbsp; 
							每页<font color="#930000">${page.pageSize}</font>条&nbsp;&nbsp; 
						</h3>
