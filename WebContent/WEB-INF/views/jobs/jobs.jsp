	...
	<section>
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				<h2>
					<i class="fa fa-list"></i> 爬取文章管理 <span>查看已提交的申请</span>
				</h2>
			</div>
							...
							<table class="table table-success table-striped mb30">
								...
								<tbody>
									<c:forEach items="${page.list }" var="job">
										<tr>
											<td><a href="${fn:escapeXml(job.originUrl) }" target="_blank">${fn:escapeXml(job.title) }</a></td>
											<td>${fn:escapeXml(job.category) }</td>
											<td>${fn:substring(job.submitTime,0,16) }</td>
											<td>
											   <c:choose>
											...
											<td>${fn:escapeXml(job.comment) }</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
