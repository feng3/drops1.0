								...
								<tbody>
									<c:forEach items="${page.list }" var="job">
										<tr>
											...
											<td>${fn:escapeXml(job.submitUserName) }</td>
											<td>
											   <c:choose>
											...
											<td><a href="./jobs.do?method=showSingleJob&action=accept&id=${job.id}" target="_blank">通过</a>&nbsp;&nbsp;<a href="./jobs.do?method=doManageJobs&action=reject&id=${job.id}">拒绝</a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
					...
					<div class="col-md-12">
					  <h3>
						...
						<c:if test="${page.hasPrev }">
							<a class="btn btn-darkblue" href="jobs.do?method=manageJobs">首页</a>
							<a class="btn btn-primary" href="jobs.do?method=manageJobs&pageNo=${page.prevPage }">上一页</a>
						</c:if>
						<c:if test="${page.hasNext }">
							<a class="btn btn-darkblue" href="jobs.do?method=manageJobs&pageNo=${page.nextPage }">下一页</a>
							<a class="btn btn-primary" href="jobs.do?method=manageJobs&pageNo=${page.totalPageNumber }">末页</a>
						</c:if>
					  </h3>
 						<h3>
